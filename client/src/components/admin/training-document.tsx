...
import { File, Upload, MoreVertical, DownloadCloud } from "lucide-react";
import { 
  Card,
  CardContent,
  CardFooter,
...
export function TrainingDocument() {
  ...
  const handleFileSubmit = async () => {
    ...
    try {
      ...
      formData.append("name", `Document training ${selectedFile.name}`);
    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-medium mb-2">
          {t("admin.training.newDocumentTraining")}
        </h2>
          ...
          <div className="border-2 border-dashed border-neutral-200 rounded-lg p-6 text-center">
            ...
            {selectedFile ? (
                ...
                <div className="flex justify-center gap-2">
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      t("admin.training.uploadDocument")
                    )}
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <p className="font-medium">{t("admin.training.dragOrClickUpload")}</p>
                  <p className="text-sm text-neutral-500">
                    {t("admin.training.supportedFormats")}
                  </p>
                </div>
                <Button
                  type="button"
      ...
      <div>
        <h2 className="text-lg font-medium mb-2">
          {t("admin.training.trainedDocuments")}
        </h2>
        ...
        <div className="space-y-4">
          {documentsLoading ? (
            ...
            <div className="text-center py-8 bg-neutral-50 rounded-lg border border-dashed">
              <File className="h-12 w-12 text-neutral-300 mx-auto mb-3" />
                ...
                <Card key={doc.id} className="bg-white border shadow-sm">
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center text-sm">
                      ...
                      <span className="truncate">{doc.file_path?.split('/').pop() || doc.name}</span>
                    </div>
                  </CardContent>
